# S181 — Capacités asynchrones (Synopsis vidéo) — design

**Sprint** : S181. **Briques** : `synopsis` (port 6090, version 1.1.0 → 1.2.0)
...
puis `GET {base}/{id}`). **S181 généralise ce contrat à TOUTE capacité
...
- **Annulation des jobs en cours** : non gérée en S181 (scop volontairement
...
- Persistance du `job_id` dans le fil d'activité (`core/fil_activite.py`) — S181
