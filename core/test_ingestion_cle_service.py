"""S211 — le Cœur porte INGESTION_KEY sur TOUS ses appels à la brique `ingestion`.

La brique `ingestion` est passée de « grande ouverte sur 5200 » à « fermée par API_KEYS ».
Le contrat manifeste (capacités `ingestion_*`) passe déjà par `outils_communs._entetes_brique`,
...
def _cles_ingestion(faux):
    """Clés X-API-Key vues sur les appels visant l'ingestion (au moins un appel exigé)."""
    vus = [h.get("X-API-Key") for m, u, h in faux.appels if "ingestion" in u]
    assert vus, f"aucun appel vers l'ingestion capturé — appels : {faux.appels}"
    return vus
...
def test_entetes_brique_derive_le_nom_de_variable(monkeypatch):
    monkeypatch.setenv("INGESTION_KEY", CLE)
    assert orchestrateur.entetes_brique("ingestion") == {"X-API-Key": CLE}
...
def test_entetes_brique_sans_cle_ne_pose_pas_den_tete(monkeypatch):
    ...
    assert orchestrateur.entetes_brique("ingestion") == {}


def test_outils_communs_utilise_la_meme_source(monkeypatch):
    monkeypatch.setenv("INGESTION_KEY", CLE)
    assert outils_communs._entetes_brique("ingestion")["X-API-Key"] == CLE
...
def test_usine_ingestion_porte_la_cle(monkeypatch):
    ...
    _run(orchestrateur._etape_ingestion(
        faux, "http://ingestion", [("a.pdf", b"x", "application/pdf")], "liv-1"))

    assert _cles_ingestion(faux) == [CLE]
...
def test_usine_ingestion_sans_fichier_porte_la_cle(monkeypatch):
    ...
    _run(orchestrateur._etape_ingestion(faux, "http://ingestion", [], "liv-1"))
...
def test_cycle_de_vie_reinjecter_porte_la_cle(monkeypatch):
    ...
    monkeypatch.setattr(cycle_de_vie, "_bases", lambda registre: {
        "ingestion": "http://ingestion", "audit": "http://audit",
...
def test_proactif_documents_a_classer_porte_la_cle(monkeypatch):
    ...
    monkeypatch.setattr(proactif.orchestrateur, "_brique_base",
                        lambda registre, nom: "http://ingestion")
    faux = _FauxClient({"/documents": {"documents": []}})
    monkeypatch.setattr(proactif.httpx, "AsyncClient", faux)

    _run(proactif._check_documents(SimpleNamespace(briques={"ingestion": {"port": 5200}})))
...
def test_outils_documents_portent_la_cle(monkeypatch):
    ...
    for outil, args in (
        ...
        assert _cles_ingestion(faux) == [CLE], f"outil {outil} sans clé"
