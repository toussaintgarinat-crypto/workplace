...
from services import google_calendar, google_oauth
...
async def google_connect(user: dict = Depends(get_current_user)):
    ...
    _require_configured()
    return {"authorization_url": google_oauth.build_auth_url(state=user["sub"])}
...
async def google_callback(
    ...
    """Callback OAuth Google. `state` porte l'identité de l'utilisateur connecté.

    Note : Google appelle cette URL sans le JWT de la brique ; l'identité provient
    donc du `state` signé côté connect. En déploiement, durcir avec un state
    opaque vérifié (cf. dette LIVE du sprint).
    """
    _require_configured()
    try:
