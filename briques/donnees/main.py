...
from auth import config_valide, garde_auth
...
def _init_db():
    with _connexion() as conn:
        ...
        """)
        conn.execute(
            "CREATE INDEX IF NOT EXISTS idx_app_entite ON enregistrements(app_id, entite_id)"
        )
        conn.commit()
    ...
    with _connexion() as conn:
        for app_id, entites in (graine or {}).items():
            for entite_id, recs in (entites or {}).items():
                deja = conn.execute(
                    "SELECT COUNT(*) AS n FROM enregistrements WHERE app_id=? AND entite_id=?",
                    (app_id, entite_id),
                ).fetchone()["n"]
                if deja:
                    continue
                ...
                for d in (recs or []):
                    ...
                    conn.execute(
                        "INSERT INTO enregistrements (id, app_id, entite_id, donnees, date_creation, date_maj) "
                        "VALUES (?,?,?,?,?,?)",
                        (str(uuid.uuid4()), app_id, entite_id,
...
def lister(app_id: str, entite_id: str):
    with _connexion() as conn:
        rows = conn.execute(
            "SELECT * FROM enregistrements WHERE app_id=? AND entite_id=? "
...
def creer(app_id: str, entite_id: str, donnees: dict):
    ...
    with _connexion() as conn:
        conn.execute(
            "INSERT INTO enregistrements (id, app_id, entite_id, donnees, date_creation, date_maj) "
            "VALUES (?,?,?,?,?,?)",
            (rec_id, app_id, entite_id, json.dumps(donnees, ensure_ascii=False), now, now),
        )
        conn.commit()
...
def modifier(app_id: str, entite_id: str, rec_id: str, donnees: dict):
    ...
    with _connexion() as conn:
        cur = conn.execute(
            ...
            "WHERE id=? AND app_id=? AND entite_id=?",
            (json.dumps(donnees, ensure_ascii=False), now, rec_id, app_id, entite_id),
        )
        conn.commit()
        if cur.rowcount == 0:
...
def supprimer(app_id: str, entite_id: str, rec_id: str):
    with _connexion() as conn:
        cur = conn.execute(
            "DELETE FROM enregistrements WHERE id=? AND app_id=? AND entite_id=?",
            (rec_id, app_id, entite_id),
        )
        conn.commit()
        if cur.rowcount == 0:
            raise HTTPException(404, "Enregistrement introuvable")
...
def semer(app_id: str, entite_id: str, graine: Graine):
...
def exporter_app(app_id: str):
    ...
    with _connexion() as conn:
        rows = conn.execute(
            "SELECT * FROM enregistrements WHERE app_id=? ORDER BY entite_id, date_creation ASC",
            (app_id,),
        ).fetchall()
...
def importer_app(app_id: str, payload: ImportDonnees):
        ...
        for entite_id, recs in payload.entites.items():
            for d in recs:
                ...
                conn.execute(
                    ...
                    "(id, app_id, entite_id, donnees, date_creation, date_maj) "
                    "VALUES (?,?,?,?,?,?)",
                    (rec_id, app_id, entite_id,
...
def resume(app_id: str):
    """Compte les enregistrements par entité pour une app (utile au tableau de bord)."""
    with _connexion() as conn:
        rows = conn.execute(
            "SELECT entite_id, COUNT(*) AS n FROM enregistrements WHERE app_id=? "
            "GROUP BY entite_id",
            (app_id,),
        ).fetchall()
...
def purger_app(app_id: str):
    """Efface toutes les données d'une app (désinstallation / remise à zéro)."""
    with _connexion() as conn:
        conn.execute("DELETE FROM enregistrements WHERE app_id=?", (app_id,))
        conn.commit()
