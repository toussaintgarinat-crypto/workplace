...
def _compte(email: str) -> dict:
    r = client.post("/inscription", json={"email": email, "mot_de_passe": "motdepasse123",
                                          "pseudo": email.split("@")[0]})
    assert r.status_code == 200
    return {jeton.COOKIE_NOM: r.cookies.get(jeton.COOKIE_NOM)}
...
def test_personnage_invisible_pour_un_autre_compte(monkeypatch):
    ...
    cookies_a = _compte("tenant-a@example.com")
    cookies_b = _compte("tenant-b@example.com")
    r = client.post("/personnages", json={"nom": "Secret", "date_naissance": "1990-01-01"},
                    cookies=cookies_a)
    pid = r.json()["id"]
    assert client.get(f"/personnages/{pid}", cookies=cookies_a).status_code == 200
    assert client.get(f"/personnages/{pid}", cookies=cookies_b).status_code == 404
    assert not any(p["id"] == pid for p in
                  client.get("/personnages", cookies=cookies_b).json())


def test_zones_identiques_pour_tous_les_comptes():
    zones.seed_zones()
    cookies_a = _compte("tenant-c@example.com")
    cookies_b = _compte("tenant-d@example.com")
    a = client.get("/zones", cookies=cookies_a).json()
    b = client.get("/zones", cookies=cookies_b).json()
...
def test_etapes_archetype_identiques_pour_tous_les_comptes():
    ...
    cookies_a = _compte("tenant-e@example.com")
    cookies_b = _compte("tenant-f@example.com")
    a = client.get("/archetypes/Le Sage Contemplatif/etapes", cookies=cookies_a).json()
    b = client.get("/archetypes/Le Sage Contemplatif/etapes", cookies=cookies_b).json()
    assert [e["id"] for e in a] == [e["id"] for e in b]


def test_groupe_dun_compte_pas_manipulable_par_un_autre(monkeypatch):
    _patch_moteur(monkeypatch)
    cookies_c = _compte("tenant-g@example.com")
    cookies_d = _compte("tenant-h@example.com")
    r = client.post("/personnages", json={"nom": "AutreCompte", "date_naissance": "1990-01-01"},
                    cookies=cookies_c)
    pid = r.json()["id"]
    archetypes.seed_zones_archetype()
    etape = client.get("/archetypes/Le Sage Contemplatif/etapes", cookies=cookies_c).json()[0]
...
def test_deux_comptes_avec_le_meme_pseudo_restent_distincts(monkeypatch):
    ...
    comptes.email, pas sur comptes.pseudo). Les deux comptes ci-dessous partagent
    volontairement le même pseudo pour le prouver."""
    _patch_moteur(monkeypatch)
    r_a = client.post("/inscription", json={"email": "meme-pseudo-a@example.com",
                                             "mot_de_passe": "motdepasse123", "pseudo": "MemePseudo"})
    assert r_a.status_code == 200
    r_b = client.post("/inscription", json={"email": "meme-pseudo-b@example.com",
                                             "mot_de_passe": "motdepasse123", "pseudo": "MemePseudo"})
    assert r_b.status_code == 200
    cookies_a = {jeton.COOKIE_NOM: r_a.cookies.get(jeton.COOKIE_NOM)}
    cookies_b = {jeton.COOKIE_NOM: r_b.cookies.get(jeton.COOKIE_NOM)}
    perso = client.post("/personnages", json={"nom": "PersoA", "date_naissance": "1990-01-01"},
                        cookies=cookies_a)
    pid = perso.json()["id"]
    assert client.get(f"/personnages/{pid}", cookies=cookies_a).status_code == 200
    assert client.get(f"/personnages/{pid}", cookies=cookies_b).status_code == 404
